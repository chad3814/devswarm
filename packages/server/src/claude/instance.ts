...
export type ClaudeRole = 'main' | 'spec_creator' | 'worker';
...
export class ClaudeInstance extends EventEmitter {
    ...
    async start(): Promise<void> {
        ...
        // Start completion polling for worker role
        if (role === 'worker' && this.options.contextId) {
    ...
    private checkOwnCompletion(): void {
        if (this.role !== 'worker' || !this.options.contextId) {
            return;
        }
    ...
    private handleTimeout(): void {
        ...
        // If this is a worker, mark the spec with an error
        if (this.role === 'worker' && this.options.contextId) {
            ...
            if (spec) {
                ...
                this.options.db.updateSpec(this.options.contextId, {
                    error_message: `Worker instance exceeded maximum runtime (${this.options.maxRuntime || 2 * 60 * 60 * 1000}ms)`,
                    status: 'error',
                });
            }
        }
